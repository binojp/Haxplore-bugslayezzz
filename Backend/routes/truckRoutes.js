...
router.post("/admin/truck-routes/create", authMiddleware, roleMiddleware(["admin", "superadmin"]), async (req, res) => {
  try {
    const { threshold = 95 } = req.body;
    
    // Find all bins at or above threshold
    const fullBins = await findFullBins(threshold);
    
    if (fullBins.length === 0) {
      return res.status(404).json({ message: "No bins found at or above threshold" });
    }

    // Group bins by proximity
    const binGroups = groupBinsByProximity(fullBins, 5);
    const createdRoutes = [];

    // Create a route for each group
    for (const group of binGroups) {
      // Find nearest available worker
      const worker = await findNearestWorker(group);
      
      if (!worker) {
        console.warn("No available worker found for bin group");
        continue;
      }

      // Optimize route order
      const { bins: optimizedBins, totalDistance } = optimizeRoute(group);
    ...
    res.status(201).json({
      message: `Created ${createdRoutes.length} optimized route(s)`,
