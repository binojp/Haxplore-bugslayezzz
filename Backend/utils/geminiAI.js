...
async function generateEducationalContent(topic) {
  try {
    ...
    const prompt = `Generate educational content about e-waste disposal for the topic: "${topic}".
...
async function generateQuizQuestions(topic, numQuestions = 5) {
  try {
    ...
    const prompt = `Generate ${numQuestions} multiple-choice quiz questions about e-waste disposal for the topic: "${topic}".
    ...
    Make questions educational and practical for users learning about proper e-waste disposal.`;
