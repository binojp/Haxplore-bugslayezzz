...
const createCustomIcon = (type, level, isPulse = false) => {
  ...
  if (type === "phone") { IconComponent = Smartphone; color = "#3b82f6"; }
  else if (type === "battery" || type === "batteries") { IconComponent = Battery; color = "#f59e0b"; }
  else if (type === "laptop") { IconComponent = Laptop; color = "#6366f1"; }
  else if (type === "charger") { IconComponent = Zap; color = "#10b981"; }
  else if (type === "cable") { IconComponent = Cpu; color = "#a855f7"; }
...
// --- FETCH BINS ---
  ...
  useEffect(() => {
    if (navigator.geolocation) {
      const watchId = navigator.geolocation.watchPosition(
        ...
        () => setUserLocation([9.5916, 76.5221]),
  ...
  if (!selectedType && !isEmbedded) {
    const types = [
      { id: "battery", name: "Batteries", icon: Battery, color: "text-amber-400", bg: "bg-amber-400/10" },
      { id: "phone", name: "Phones", icon: Smartphone, color: "text-blue-400", bg: "bg-blue-400/10" },
  ...
  return (
    <div className={`${isEmbedded ? "relative w-full h-full" : "fixed inset-0 h-[100dvh] theme-bg theme-text flex flex-col md:flex-row pt-16"}`}>
      ...
      <div className="flex-1 relative">
        <MapContainer
          center={userLocation || [9.5916, 76.5221]}
          zoom={14}
