...
export default function Profile() {
  ...
  const badges = [
    ...
    { name: "E-Waste Warrior", description: "500+ Green Credits", earned: (userData?.totalPoints || 0) >= 500, icon: Zap },
  ...
  return (
    <div className="min-h-screen theme-bg theme-text p-4 pt-24 pb-24 overflow-x-hidden">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="relative">
              ...
              <div className="mt-6 flex justify-between border-t border-white/10 pt-4">
                 <div className="text-center">
                    <p className="text-2xl font-bold text-white">{userData?.totalPoints || 0}</p>
                    <p className="text-[10px] text-gray-500 uppercase tracking-wider">Credits</p>
                 </div>
                 <div className="text-center border-l border-white/10 pl-6">
                    <p className="text-2xl font-bold text-emerald-400">{((userData?.totalPoints || 0) / 10).toFixed(1)}</p>
              ...
              {badges.map((badge) => (
                <div 
                  ...
                  className={`p-3 rounded-xl border flex flex-col items-center text-center gap-2 transition-all ${
                    ...
                    : "bg-white/5 border-white/5 text-gray-600 grayscale opacity-50"
                  }`}
                >
                  <badge.icon size={20} className={badge.earned ? "text-emerald-400" : "text-gray-500"} />
                  <div>
                    <p className="text-xs font-bold leading-tight">{badge.name}</p>
                    {badge.earned && <p className="text-[10px] text-emerald-300/80 mt-1">Unlocked</p>}
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* RIGHT COLUMN: Collections & Forms */}
        <div className="lg:col-span-2 space-y-6">
          
          {/* HOME ADDRESS */}
          <div className="glass-panel p-6 rounded-3xl group hover:border-emerald-500/30 transition-all">
             <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                   ...
                   <div>
                      <h3 className="font-bold text-lg">My Recycling Hub</h3>
                      <p className="text-xs text-gray-400">Manage pickup location</p>
                   </div>
                </div>
                {!homeAddress && (
                  <button onClick={() => setShowAddHome(true)} className="px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-bold rounded-xl transition-all">
                    + Add New
                  </button>
                )}
             </div>
             ...
             {homeAddress ? (
               <div className="bg-white/5 p-4 rounded-xl border border-white/10 flex justify-between items-center">
                  <div>
                    <p className="text-sm text-gray-200">{homeAddress.address}</p>
                    <p className="text-xs text-gray-500 mt-1">{homeAddress.city}, {homeAddress.pincode}</p>
                  </div>
                  <button onClick={() => setShowAddHome(true)} className="text-xs text-emerald-400 font-bold hover:text-emerald-300 underline">
                    Edit
                  </button>
               </div>
             ) : (
          ...
          <div className="glass-panel p-6 rounded-3xl min-h-[400px]">
             ...
             <div className="space-y-3">
               {collections.length > 0 ? (
                 collections.map((col) => (
                   <div key={col._id} className="p-4 bg-white/5 border border-white/5 rounded-2xl flex justify-between items-center hover:bg-white/10 transition-all">
                      <div className="flex gap-4 items-center">
                         <div className="text-center bg-white/5 p-2 rounded-lg">
                           <p className="text-xs font-bold text-gray-400 uppercase">{new Date(col.scheduledDate).toLocaleString('default', { month: 'short' })}</p>
                           <p className="text-lg font-black text-white">{new Date(col.scheduledDate).getDate()}</p>
                         </div>
                         <div>
                            <p className="font-bold text-sm text-white">{col.wasteType} Collection</p>
                            <p className="text-xs text-gray-500">{col.notes || "No additional notes"}</p>
                         </div>
                      </div>
                      <span className={`px-3 py-1 text-[10px] font-bold uppercase rounded-full border ${getStatusColor(col.status)}`}>
                        {col.status}
                      </span>
                   </div>
                 ))
                 ...
                 <div className="text-center py-12">
                   <p className="text-gray-600 text-sm">No active pickups. Schedule one to get started!</p>
                 </div>
               )}
             </div>
          </div>
      ...
      {/* MODALS styling (Keeping simplified structure for brevity but applying dark theme classes) */}
      {showAddHome && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[1000] p-4">
          <div className="glass-panel w-full max-w-md p-6 rounded-3xl border border-white/10">
            <h3 className="text-xl font-bold text-white mb-4">My E-Waste Hub</h3>
            <form onSubmit={handleAddHome} className="space-y-4">
              <textarea 
                placeholder="Street Address"
                value={formData.address}
                onChange={(e) => setFormData({ ...formData, address: e.target.value })}
                className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white placeholder:text-gray-600 focus:outline-none focus:border-emerald-500"
                required
              />
              <div className="grid grid-cols-2 gap-3">
                 <input 
              ...
              <button type="button" onClick={handleTrackLocation} className="w-full py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-emerald-400 text-xs font-bold flex items-center justify-center gap-2">
              ...
              <div className="flex gap-3 mt-4">
                <button type="button" onClick={() => setShowAddHome(false)} className="flex-1 py-3 bg-white/5 text-gray-400 font-bold rounded-xl hover:bg-white/10">Cancel</button>
      ...
      {showScheduleCollection && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[1000] p-4">
          <div className="glass-panel w-full max-w-md p-6 rounded-3xl border border-white/10">
            <h3 className="text-xl font-bold text-white mb-4">Schedule Pickup</h3>
            <form onSubmit={handleScheduleCollection} className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white mt-1 focus:outline-none focus:border-emerald-500"
                  required
                />
              </div>

              <div>
                <label className="text-xs text-gray-400 font-bold uppercase ml-1">E-Waste Type</label>
                  <select 
                    value={formData.wasteType}
                    onChange={(e) => setFormData({ ...formData, wasteType: e.target.value })}
                    className="w-full bg-slate-800 border border-white/10 rounded-xl p-3 text-white mt-1 focus:outline-none focus:border-emerald-500"
                  >
              ...
              <div className="flex gap-3 mt-4">
                <button type="button" onClick={() => setShowScheduleCollection(false)} className="flex-1 py-3 bg-white/5 text-gray-400 font-bold rounded-xl hover:bg-white/10">Cancel</button>
